...
Boomerang::Boomerang(double alt, Eigen::Vector3d A, Eigen::Vector3d B,
...
void Boomerang::generateTraj(std::vector<snapstack_msgs2::msg::Goal>& goals,
    ...
    goals.push_back(createLineGoal(B_.x(), B_.y(), -v, 0, theta_));

    //for(int i = 0; i < v_goals_.size(); ++i){
    v_goal = v_goals_[0];  // TODO
    index_msgs[goals.size() - 1] = "Line traj: accelerating to " + std::to_string(v_goal) + " m/s";
    // accelerate to the goal velocity

    while(fabs(v) < fabs(v_goal)){
        // generate points in the line with *increasing* velocity
        // RCLCPP_INFO(logger_, "v: %f", v);
        v = std::max(v - a1_*dt_, -v_goal);
        goals.push_back(createLineGoal(goals.back().p.x, goals.back().p.y, v, -a1_, theta_));
    }
    ...
    //    std::cout << "(" << goal.p.x << ", " << goal.p.y << ")" << std::endl;
    //}
    if(fabs(A_.x() - goals.back().p.x) > thresh or fabs(A_.y() - goals.back().p.y) > thresh){
        RCLCPP_ERROR(logger_, "Error: final point is not A");
        exit(1);
    }
    // Force last goal pos to be equal to B
    goals.back().p.x = A_.x();
...
snapstack_msgs2::msg::Goal Boomerang::createLineGoal(double last_x, double last_y, double v, double accel, double theta) const{
...
void Boomerang::generateStopTraj(std::vector<snapstack_msgs2::msg::Goal>& goals,
                            std::unordered_map<int,std::string>& index_msgs,
                            int& pub_index,
                            const rclcpp::Clock::SharedPtr& clock){
    // rclcpp::Time tstart = rclcpp::Time::now();
...
bool Boomerang::trajectoryInsideBounds(double xmin, double xmax,
...
double Boomerang::get_d2() const{  // get length of constant velocity segment
