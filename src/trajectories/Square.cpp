...
Square::Square(double alt, double side_length, double cx, double cy, double orientation,
           std::vector<double> v_goals, double t_traj, double accel, double dt)
    : alt_(alt), side_length_(side_length), cx_(cx), cy_(cy), orientation_(orientation),
      v_goals_(v_goals), t_traj_(t_traj), accel_(accel), Trajectory(dt) {}

Square::~Square() {}

void Square::generateTraj(std::vector<snapstack_msgs2::msg::Goal>& goals,
                          std::unordered_map<int,std::string>& index_msgs,
                          const rclcpp::Clock::SharedPtr& clock)
{
    rclcpp::Time tstart = clock->now();

    // Calculate the four corners of the square (counter-clockwise, starting at top-left)
    double half = side_length_ / 2.0;
    std::vector<Eigen::Vector2d> corners(4);
    // Unrotated corners (relative to center)
    corners[0] = Eigen::Vector2d(-half,  half); // top-left
    corners[1] = Eigen::Vector2d( half,  half); // top-right
    corners[2] = Eigen::Vector2d( half, -half); // bottom-right
    corners[3] = Eigen::Vector2d(-half, -half); // bottom-left
    ...
    double total_perimeter = 4 * side_length_;
    ...
    goals.push_back(createSquareGoal(x, y, v_goal, 0, heading));
    index_msgs[goals.size() - 1] = "Square traj: starting at corner 0";
    ...
    for (int lap = 0; lap < num_laps; ++lap) {
        for (int side = 0; side < 4; ++side) {
            ...
            for (int step = 1; step <= steps && t < t_traj_; ++step) {
                ...
                goals.push_back(createSquareGoal(x_new, y_new, v_goal, 0, heading));
                index_msgs[goals.size() - 1] = "Square traj: moving along side " + std::to_string(side);
                t += dt;
                if (t >= t_traj_) break;
            }
    ...
    index_msgs[goals.size() - 1] = "Square traj: completed";
...
snapstack_msgs2::msg::Goal Square::createSquareGoal(double x, double y, double v, double accel, double heading) const {
...
void Square::generateStopTraj(std::vector<snapstack_msgs2::msg::Goal>& goals,
                              std::unordered_map<int,std::string>& index_msgs,
                              int& pub_index,
                              const rclcpp::Clock::SharedPtr& clock) {
    // Simple stop: decelerate to zero velocity at current position
    rclcpp::Time tstart = clock->now();
    ...
    stop_msgs[0] = "Square traj: pressed END, decelerating to 0 m/s";

    while (v > 0) {
        v = std::max(v - accel_ * dt_, 0.0);
        stop_goals.push_back(createSquareGoal(last_goal.p.x, last_goal.p.y, v, -accel_, heading));
    }
    stop_msgs[stop_goals.size() - 1] = "Square traj: stopped";
...
bool Square::trajectoryInsideBounds(double xmin, double xmax,
    ...
    corners[0] = Eigen::Vector3d(cx_ + c * -half - s * half, cy_ + s * -half + c * half, alt_);
    corners[1] = Eigen::Vector3d(cx_ + c * half - s * half, cy_ + s * half + c * half, alt_);
    corners[2] = Eigen::Vector3d(cx_ + c * half - s * -half, cy_ + s * half + c * -half, alt_);
    corners[3] = Eigen::Vector3d(cx_ + c * -half - s * -half, cy_ + s * -half + c * -half, alt_);
