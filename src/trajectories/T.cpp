...
T::T(double cx, double cy, double length, double width, double alt,
...
void T::generateTraj(std::vector<snapstack_msgs2::msg::Goal>& goals,
                     std::unordered_map<int,std::string>& index_msgs,
                     const rclcpp::Clock::SharedPtr& clock)
{
    rclcpp::Time tstart = clock->now();

    // Define the 4 points of the T in local coordinates (centered at 0,0)
    // Start at left of top bar, right of top bar, center top, then down to bottom of vertical
    std::vector<Eigen::Vector2d> points = {
        {-width_/2,  length_/2}, // left of top bar
        { width_/2,  length_/2}, // right of top bar
        {    0.0,    length_/2}, // center top
        {    0.0,   -length_/2}  // bottom of vertical
    };
    ...
    while (t < t_traj_) {
        ...
        for (size_t seg = 0; seg < path.size() - 1 && t < t_traj_; ++seg) {
            ...
            for (int i = 0; i <= steps && t < t_traj_; ++i) {
                ...
                goals.push_back(createTGoal(x, y, v_goal, 0, heading));
                index_msgs[goals.size() - 1] = std::string("T traj: segment ") + std::to_string(seg) + (forward ? " fwd" : " rev");
                t += dt;
            }
        }
    ...
    index_msgs[goals.size() - 1] = "T traj: completed";
    RCLCPP_INFO(logger_, "T traj generated in %f s, %lu goals", (clock->now() - tstart).seconds(), goals.size());
}

snapstack_msgs2::msg::Goal T::createTGoal(double x, double y, double v, double accel, double heading) const
{
...
void T::generateStopTraj(std::vector<snapstack_msgs2::msg::Goal>& goals,
                         std::unordered_map<int,std::string>& index_msgs,
                         int& pub_index,
                         const rclcpp::Clock::SharedPtr& clock)
{
    ...
    stop_msgs[0] = "T traj: pressed END, decelerating to 0 m/s";

    double decel = 1.0; // You can make this a parameter if desired
    while (v > 0) {
        v = std::max(v - decel * dt_, 0.0);
        stop_goals.push_back(createTGoal(last_goal.p.x, last_goal.p.y, v, -decel, heading));
    }
    stop_msgs[stop_goals.size() - 1] = "T traj: stopped";
...
bool T::trajectoryInsideBounds(double xmin, double xmax,
                              double ymin, double ymax,
                              double zmin, double zmax)
{
    // Check all 4 points of the T
